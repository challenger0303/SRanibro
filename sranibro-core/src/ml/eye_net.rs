...
const SPEC: &[(&str, &[i64])] = &[
    ("conv1_weight", &[20, 2, 5, 5]),  ("conv1_bias", &[20]),
    ("conv2_weight", &[48, 20, 5, 5]), ("conv2_bias", &[48]),
    ("conv3_weight", &[64, 48, 3, 3]), ("conv3_bias", &[64]),
    ("fc5_ft_weight", &[500, 25600]),  ("fc5_ft_bias", &[500]),
    ("fc6_10_weight", &[5, 500]),      ("fc6_10_bias", &[5]),
];
...
impl EyeNet {
    ...
    pub fn new(map: HashMap<String, Tensor>) -> Result<Self, String> {
        ...
        Ok(Self {
            conv1_w: take("conv1_weight"), conv1_b: take("conv1_bias"),
            conv2_w: take("conv2_weight"), conv2_b: take("conv2_bias"),
            conv3_w: take("conv3_weight"), conv3_b: take("conv3_bias"),
            fc1_w: take("fc5_ft_weight"), fc1_b: take("fc5_ft_bias"),
            fc2_w: take("fc6_10_weight"), fc2_b: take("fc6_10_bias"),
            col: Vec::new(),
            a: Vec::new(),
            b: Vec::new(),
        })
    ...
    pub fn forward_one(&mut self, input: &[f32]) -> [f32; OUT_DIM] {
        ...
        let (h, w) = conv_relu(input, 2, 100, 100, &self.conv1_w, 20, 5, &self.conv1_b,
                               &mut self.col, &mut self.a);
        let (h, w) = maxpool2(&self.a, 20, h, w, &mut self.b);

        // conv2+relu (b -> a), maxpool (a -> b): [20,48,48]->[48,44,44]->[48,22,22]
        let (h, w) = conv_relu(&self.b, 20, h, w, &self.conv2_w, 48, 5, &self.conv2_b,
                               &mut self.col, &mut self.a);
        let (h, w) = maxpool2(&self.a, 48, h, w, &mut self.b);

        // conv3+relu (b -> a): [48,22,22]->[64,20,20]
        let _ = conv_relu(&self.b, 48, h, w, &self.conv3_w, 64, 3, &self.conv3_b,
                          &mut self.col, &mut self.a);

        // fc1 (a -> b), fc2 (b -> a)
        linear_relu(&self.a, &self.fc1_w, &self.fc1_b, 500, 64 * 20 * 20, &mut self.b);
...
fn im2col(inp: &[f32], ic: usize, ih: usize, iw: usize, k: usize, col: &mut Vec<f32>)
    -> (usize, usize) {
    let oh = ih - k + 1;
    let ow = iw - k + 1;
    let kk = k * k;
...
fn conv_relu(
    ...
    w: &[f32], oc: usize, k: usize, bias: &[f32],
    col: &mut Vec<f32>, out: &mut Vec<f32>,
) -> (usize, usize) {
...
fn linear_relu(inp: &[f32], w: &[f32], bias: &[f32], out_dim: usize, in_dim: usize,
               out: &mut Vec<f32>) {
    ensure(out, out_dim);
    unsafe {
        matrixmultiply::sgemm(
